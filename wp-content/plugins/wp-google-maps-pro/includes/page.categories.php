...
function wpgmza_return_marker_count_by_category( $cat_id = false, $map_id = false )
{
	...
	{
		$categoryTree = new \WPGMZA\CategoryTree();
	}
...
function wpgmaps_list_categories()
{
    ...
    $results = $wpdb->get_results("SELECT * FROM `$wpgmza_tblname_categories` WHERE `active` = 0 AND `parent` = '0' ORDER BY " . \WPGMZA\Category::getOrderBy());
    $ret_msg = "<table id='wpgmza-category-table' class=\"wp-list-table widefat fixed striped pages\">";
    $ret_msg .= "  <thead>";
    $ret_msg .= "      <tr>";
        ...
        while ($continue) {
            ...
            foreach ($parents_to_check as $current_parent_id => $has_been_checked) {
                if (!$has_been_checked) {
                    ...
                    $sql = "SELECT * FROM `$wpgmza_tblname_categories` WHERE `active` = 0 AND `parent` = '".$current_parent_id."' ORDER BY " . \WPGMZA\Category::getOrderBy();
