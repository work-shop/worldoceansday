...
class CustomFieldsPage
{
	...
	public static function POST()
	{
		...
		for($i = 0; $i < $numFields; $i++)
		{
			...
			$display_in_infowindows		= isset($_POST['display_in_infowindows']) && in_array($id, (array) $_POST['display_in_infowindows']) || ($id == -1 && isset( $_POST['display_in_infowindows']['-1'] ));
			$display_in_marker_listings	= isset($_POST['display_in_marker_listings']) && in_array($id, (array) $_POST['display_in_marker_listings']);
			...
			if($id == -1 || empty($id))
			{
				...
				$qstr = "INSERT INTO $WPGMZA_TABLE_NAME_CUSTOM_FIELDS (name, icon, attributes, widget_type, display_in_infowindows, display_in_marker_listings) VALUES (%s, %s, %s, %s, %s, %s)";
				$params = array($name, $icon, $attributes, $widget_type, $display_in_infowindows, $display_in_marker_listings);
			}
			else
			{
				$qstr = "UPDATE $WPGMZA_TABLE_NAME_CUSTOM_FIELDS SET name=%s, icon=%s, attributes=%s, widget_type=%s, display_in_infowindows=%s, display_in_marker_listings=%s WHERE id=%s";
				$params = array($name, $icon, $attributes, $widget_type, $display_in_infowindows, $display_in_marker_listings, $id);
			}
	...
	protected function tableBodyHTML()
	{
		global $wpdb;
		global $WPGMZA_TABLE_NAME_CUSTOM_FIELDS;
		$query = current_user_can( 'administrator' ) ? "SELECT * FROM $WPGMZA_TABLE_NAME_CUSTOM_FIELDS" : "SELECT * FROM $WPGMZA_TABLE_NAME_CUSTOM_FIELDS WHERE display_in_infowindows = 1 OR display_in_marker_listings = 1;";
		...
		foreach($fields as $obj)
		{
			...
			<tr>
				...
				<td>
					...
					<input type="checkbox" name="display_in_infowindows[]" id="wpgmza-display-in-infowindows-<?php echo $obj->id; ?>" class="wpgmza-toggle-infowindow-display-input" <?php echo $obj->display_in_infowindows ? 'checked' : ''; ?> value="<?php echo $obj->id; ?>" />
				</td>
				<td>
					...
					<input type="checkbox" name="display_in_marker_listings[]" id="wpgmza-display-in-marker-listings<?php echo $obj->id; ?>" class="wpgmza-toggle-marker-listing-display-input" <?php echo $obj->display_in_marker_listings ? 'checked' : ''; ?> value="<?php echo $obj->id; ?>" />
				</td>
				<td>
				...
				<tbody>
					...
					<tr id="wpgmza-new-custom-field">
						...
						<td>
							...
							<input type="checkbox" name="display_in_infowindows[]" id="wpgmza-display-in-infowindows--1" class="wpgmza-toggle-infowindow-display-input" checked value="-1"/>
						</td>
						<td>
							<label class="wpgmza-display-in-marker-listings fa fa-eye" for="wpgmza-display-in-marker-listings--1"></label>
							<input type="checkbox" name="display_in_marker_listings[]" id="wpgmza-display-in-marker-listings--1" class="wpgmza-toggle-marker-listing-display-input" checked value="-1"/>
						</td>
						<td>
