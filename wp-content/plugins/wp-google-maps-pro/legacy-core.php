...
function wpgmza_pro_menu() {
    ...
    if($_GET['action'] != "wizard"){
		...
		if(!empty($map->default_marker))
			$options['value'] = $map->default_marker;
    ...
    echo "
       ...
       <div class='wrap'>
        ...
        <div id=\"wpgmaps_tabs\">
                ...
                <div id=\"tabs-1\">
                    ...
                    <table>
                        <tr>
                            ...
                            <td><input type='text' readonly name='shortcode' class='wpgmza_copy_shortcode' style='font-size:18px; text-align:center;' value='[wpgmza id=\"".$res->id."\"]' /> <small class='wpgmza-info__small'><i>".__("copy this into your post or page to display the map","wp-google-maps")."</i>
                        </td>
                        </tr>
                        <tr>
                ...
                <div id='wpgmza-map-edit__btns' class='submit'><input id='wpgmza-save-map-btn' type='submit' name='wpgmza_savemap' class='button-primary' value='".__("Save Map","wp-google-maps")." &raquo;' /></div>
	...
	if (isset($raw_response)) {
		if (!is_wp_error($raw_response) && ($raw_response['response']['code'] == 200))
		{
			...
			if($wpgmza && $wpgmza->isInDeveloperMode() && !$response)
				file_put_contents(plugin_dir_path(__FILE__) . 'update-response.json', json_encode($raw_response));
		}
...
function wpgmaps_sl_user_output_pro($map_id,$atts = false) {
	...
	// NB: Could probs pass this in
	$map = WPGMZA\Map::createInstance($map_id);
	
	if(isset($_GET['wpgmza-debug-use-new-store-locator']))
		return $map->storeLocator->html;
	...
	if($map->store_locator_search_area != "auto")
    {
    ...
    if (function_exists("wpgmza_register_pro_version") && isset($map_other_settings['store_locator_category']) && $map_other_settings['store_locator_category'] == "1") {
	    ...
	    if ($cats_override_hide) {
			...
			if($map->categoryFilterWidget instanceof \WPGMZA\CategoryFilterWidget\Dropdown)
			{
				$select = $map->categoryFilterWidget->document->querySelector("select");
				$select->addClass("wpgmza_form_field__input wpgmza_filter_select_{$map->id}");
			}
			
			$ret_msg .= $map->categoryFilterWidget->html;
...
if(!function_exists('wpgmza_get_icon'))
{
	...
	{
		$url = plugin_dir_url( __FILE__ ) . 'images/icons/' . $icon . '.png';
		return $url;
	}
}
