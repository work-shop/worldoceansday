...
jQuery(function($) {
	...
	WPGMZA.MarkerIconPicker.prototype.setIcon = function(url)
	{
		var preview = url;
...
jQuery(function($){
	...
	WPGMZA.ProMarkerPanel.prototype.populate = function(data)
	{
		...
		//$(this.element).find("[data-custom-field-id]").remove();
		
		//if(data.customFieldsAdminHTML)
		//	$(".wpgmza-save-marker-container").before($(data.customFieldsAdminHTML));
	...
	WPGMZA.ProStoreLocator.prototype.onFilteringComplete = function(event)
	{
		...
		switch(this.map.settings.store_locator_search_area)
		{
			...
			default:
				...
				// NB: Move this onto basic in 8.1.0
				if(params.center && params.radius)
				{
...
jQuery(function($) {
	...
	WPGMZA.MarkerListing.prototype.onHTMLResponse = function(html)
	{
		...
		/*$(this.element).find(".wpgmza-rating-container").each(function(index, el) {
			
			if(self.map.settings.enable_marker_ratings)
			{
				var map = self.map;
				var marker_id = $(el).closest("[data-marker-id]").attr("data-marker-id");
				var marker = map.getMarkerByID(marker_id);
				
				var widget = self.getRatingWidget(marker);
