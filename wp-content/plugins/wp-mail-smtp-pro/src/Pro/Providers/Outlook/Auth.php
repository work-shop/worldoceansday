...
class Auth extends AuthAbstract {
	...
	public function get_client() {
		...
		// Do not process if we don't have App ID/Password.
		...
		if (
			...
			try {
				...
				$access_token = $this->client->getAccessToken(
					'authorization_code', [ 'code' => $this->options['auth_code'] ]
				);
			...
			if ( ! empty( $access_token ) && $access_token->hasExpired() ) {
				...
				try {
					$new_access_token = $this->client->getAccessToken(
						'refresh_token', [ 'refresh_token' => $access_token->getRefreshToken() ]
					);
	 ...
	 *
	 * @return string
	 * @throws \Exception Emitted when something when wrong.
	 */
